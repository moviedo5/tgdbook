...
knitr::kable(t(sapply(list("`fit1`" = fit1, "`fit2`" = fit2, "`fit3`" = fit3),
    function(x) with(summary(x),
        c("$MSE$" = mean(residuals^2), "$R^2$" = r.squared, "$R^2_{adj}$" = adj.r.squared)))), "pipe",
        caption="Medidas de bondad de ajuste de los modelos polinómicos (muestra de entrenamiento).",
        digits = 2, position = "!htb")
```
